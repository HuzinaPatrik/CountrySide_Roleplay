...
function createPicker(data)
	if data then  
		...
		updatePaletteColor(data["vehicle"], data["colortype"])
		...
		if data["colortype"] == "km" then 
			localPlayer:setData("tuning >> speedoColor", true)
		end 
...
function destroyPicker()
    if isPickerRender then 
		...
		resetVehicleColors(localPlayer.vehicle)

		if pickerData["colortype"] == "km" then 
			localPlayer:setData("tuning >> speedoColor", nil)
		end 
...
function resetVehicleColors(veh)
	local data = pickerData["old"]
	if data then 
		localPlayer.vehicle:setColor(unpack(data["color"]))
		localPlayer.vehicle:setHeadLightColor(unpack(data["headlight"]))
		localPlayer.vehicle:setData("veh >> KM/H Color", data["km"])
	end 
end 
...
function drawnPicker()
	...
	if isPickerRender and progress >= 1 then 
		...
		if pickerData["vehicle"] and isElement(pickerData["vehicle"]) then
			...
			elseif pickerData["colortype"] == "km" then 
				r6, g6, b6 = selectedR, selectedG, selectedB
			end
			...
			setElementData(pickerData["vehicle"], "veh >> KM/H Color", {r6, g6, b6})
		end
	end
    ...
    local w, h = 252, 174
    local x, y = sx - w - 20, sy/2 - h/2 + respc(150)
	...
	if exports['cr_core']:isInSlot(px, py, pw, ph) then 
		selected = 1
	end 
	...
	if exports['cr_core']:isInSlot(px2, py2, pw2, ph2) then 
		selected = 2
	end 
...
function updatePaletteColor(vehicle, colortype)
	if vehicle and colortype then
		...
		local r6, g6, b6 = unpack(localPlayer.vehicle:getData("veh >> KM/H Color") or {255, 255, 255})
		...
		elseif pickerData["colortype"] == "km" then 
			vehicleColor = {r6, g6, b6}
		end

		CreateNewBar("R", {0,0,0,0}, {3, vehicleColor[1], true, tocolor(255,255,255,0), {"Poppins-Regular", 12}, 1, "center", "center", false}, 1)
		CreateNewBar("G", {0,0,0,0}, {3, vehicleColor[2], true, tocolor(255,255,255,0), {"Poppins-Regular", 12}, 1, "center", "center", false}, 2)
		CreateNewBar("B", {0,0,0,0}, {3, vehicleColor[3], true, tocolor(255,255,255,0), {"Poppins-Regular", 12}, 1, "center", "center", false}, 3)
		CreateNewBar("hex", {0,0,0,0}, {6, RGBToHex(unpack(vehicleColor)), false, tocolor(255,255,255,0), {"Poppins-Regular", 12}, 1, "center", "center", false}, 4)
