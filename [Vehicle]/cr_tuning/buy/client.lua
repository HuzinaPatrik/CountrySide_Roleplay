...
function createBuy(data)
    if data then 
        ...
        exports['cr_dx']:startFade("buyPanel", 
            {
...
function drawnBuy()
    local alpha, progress = exports['cr_dx']:getFade("buyPanel")
    if not isBuyRender then 
        if progress >= 1 then 
    ...
    dxDrawText(buyCache["tuningName"] .. subText .. " vásárlásához kérlek válasz fizetőezközt.", x, y + 40, x + w, y + 40, tocolor(242, 242, 242, alpha), 1, font2, "center", "top", false, true)
    ...
    if buyCache["tuningPrice"] then 
        ...
        tuningPrice = tuningPrices[localPlayer.vehicle.model][buyCache["tuningType"]][buyCache["nextLevel"]]
    end 
