    ...
    function(b, s)
        ...
        if s == "down" then
            ...
            for k,v in pairs(textbars) do
                ...
                if isInSlot(x,y,w,h, specBox[k]) then
                    now = k
                    textbars[now][2][2] = ""
                    
                    if now == "Admin >> search" then
                        adminSearchCache = nil
                    end
                    ...
                    if now == "Pet >> search" then 
                        PetSearchCache = nil
                        generatePetInformations(PetSelected)
                    end 

                    if now == "Options >> search" then 
                        optionsSearchCache = nil
                        ...
                        CreateNewBar("Options >> search", {0, 0, 0, 0}, {16, "", false, tocolor(242, 242, 242, 255), {"Poppins-Medium", realFontSize[12]}, 1, "left", "center", false, true}, 1)
                    end 
                    ...
                    if now == "FactionMembers >> search" then 
                        factionMembersSearchCache = nil
                        getFactionMemberInfos(factionMembersSelected)
                    end 

                    if now == "FactionRanks >> search" then 
...
function onGuiChange()
    ...
    if now == 'Group >> search' then 
        if textbars[now][2][2]:gsub(" ", "") ~= "" then
            ...
            for k, v in ipairs(cache["playerDatas"]["group"][selectedGroup]["data"][3]) do
                local text2 = string.lower(v[1])
                local text3 = string.lower(tostring(v[2]))

                if string.lower(tostring(text2)):find(text) or string.lower(text3):find(text) then
                    table.insert(groupSearchCache, v)
                    generateGroupInformations(groupSelected)
                end
            end

            if groupSelected and groupSelected > #groupSearchCache then 
                groupSelected = 1
                generateGroupInformations(groupSelected)
            end 
        else
    ...
    if now == 'Pet >> search' then 
        if textbars[now][2][2]:gsub(" ", "") ~= "" then
            ...
            for k, v in ipairs(petCache) do
                local text2 = string.lower(v[1])
                local text3 = string.lower(tostring(v[4]))
                local text4 = string.lower(tostring(petTypeNames[v[2]]))

                if string.lower(tostring(text2)):find(text) or string.lower(text3):find(text) or string.lower(text4):find(text) then
                    table.insert(PetSearchCache, v)
                    generatePetInformations(PetSelected)
                end
            end

            if PetSelected and PetSelected > #PetSearchCache then 
                PetSelected = 1
                generatePetInformations(PetSelected)
            end 
        else
    ...
    if now == "FactionMembers >> search" then
        if textbars[now][2][2]:gsub(" ", "") ~= "" then
            factionMembersSearchCache = {}
            
            --edit
            local text = string.lower(textbars[now][2][2])

            local data = cache["playerDatas"]["faction"][selectedFaction]["players"]
            for k, v in ipairs(data) do
                local text1 = v[2]:gsub("_", " ")
                local text2 = cache["playerDatas"]["faction"][selectedFaction][4][v[3]]["name"] or "Ismeretlen"

                if string.lower(tostring(text1)):find(text) or string.lower(text2):find(text) then
                    table.insert(factionMembersSearchCache, v)
                    getFactionMemberInfos(factionMembersSelected)
                end
            end

            if factionMembersSelected > #factionMembersSearchCache then 
    ...
    if now == "FactionRanks >> search" then
        if textbars[now][2][2]:gsub(" ", "") ~= "" then
            factionRanksSearchCache = {}
            
            --edit
            local text = string.lower(textbars[now][2][2])

            local data = cache["playerDatas"]["faction"][selectedFaction][4]
            for k, v in ipairs(data) do
                local text1 = v["name"]
                local text2 = v["payment"]

                if string.lower(tostring(text1)):find(text) or string.lower(text2):find(text) then
                    table.insert(factionRanksSearchCache, v)
                    getFactionRankInfos(factionRanksSelected)
                end
            end

            if factionRanksSelected > #factionRanksSearchCache then 
