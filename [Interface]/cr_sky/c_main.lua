...
function startDynamicSky()
	...
	shaderTable.skyboxTropos = dxCreateShader ( "shaders/sky/fx/shader_dynamicSky2tropos.fx", 2, 0, false, "object" )
	if dynamicSkySettings.stratosBottomStars then shaderTable.skyboxStratos = dxCreateShader ( "shaders/sky/fx/shader_dynamicSky2up.fx", 2, 0, false, "object" ) 
	    else shaderTable.skyboxStratos = dxCreateShader ( "shaders/sky/fx/shader_dynamicSky2stratos.fx", 2, 0, false, "object" ) end
	if dynamicSkySettings.stratosBottomStars then shaderTable.skyboxBottom = dxCreateShader ( "shaders/sky/fx/shader_dynamicSky2down.fx", 3, 0, false, "object" )
		else shaderTable.skyboxBottom = dxCreateShader ( "shaders/sky/fx/shader_dynamicSky2bottom.fx", 3, 0, false, "object" ) end
	shaderTable.clear = dxCreateShader ( "shaders/sky/fx/shader_clear.fx", 3, 0, false, "world" )
	textureTable.cloud = dxCreateTexture ( "shaders/sky/tex/clouds.dds", "dxt5" )
	textureTable.normal = dxCreateTexture ( "shaders/sky/tex/clouds_normal.jpg", "dxt5" ) 
	textureTable.skybox = dxCreateTexture ( "shaders/sky/tex/skybox.dds", "dxt5" )
	moonPhase = getCurrentMoonPhase()
	textureTable.moon = dxCreateTexture ( "shaders/sky/tex/moon/"..toint( 20 - toint( moonPhase * 20 ) )..".png" )
	...
	modelTable.txd = engineLoadTXD( 'shaders/sky/tex/skybox_model.txd' )
	engineImportTXD( modelTable.txd, dynamicSkySettings.modelID)
	modelTable.dff = engineLoadDFF( 'shaders/sky/dff/skybox_model.dff', dynamicSkySettings.modelID )
...
function renderTime()
	...
	if (thisWeather~=oldWeather) then
		...
		dxSetShaderValue ( shaderTable.skyboxTropos, "gCloudSpeed", 0.05 * windVelocityValue )
		dxSetShaderValue ( shaderTable.skyboxBottom, "gCloudSpeed", 0.05 * windVelocityValue )
	end
