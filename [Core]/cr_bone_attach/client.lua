...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		triggerServerEvent("requestAttachmentData", root)
		
		addEvent("attachElementToBone", true)
		addEvent("detachElementFromBone", true)
		addEventHandler("attachElementToBone", root, attachElementToBone)
		addEventHandler("detachElementFromBone", root, detachElementFromBone)
	end
)
...
setTimer(
    function()
        ...
        for k,v in pairs(attached_ped) do
            if getElementData(v, "clone") and isElement(getElementData(v, "clone")) then 
                v = getElementData(v, "clone") 
            end
            local _v = v

            if not isElement(k) then
			    clearAttachmentData(k)
		    elseif isElementStreamedIn(v) and isElementOnScreen(v) or v == localPlayer then
                renderCache[k] = _v
                ...
                if k.position.x == 0 and k.position.y == 0 and k.position.z == 0 then
                    k.alpha = 0
                else
                    setElementPosition(k, 0,0,0)
                end
            end
        end
    end, 250, 0
...
function putAttachedElementsOnBones()
	...
	for element,ped in pairs(renderCache) do
		local bone = attached_bone[element]
		if tonumber(bone) and bone_0[bone] then
...
addEventHandler("onClientPedsProcessed", root, putAttachedElementsOnBones)
...
addEventHandler("receiveAttachmentData", root, getAttachmentData)
