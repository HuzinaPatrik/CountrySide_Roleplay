...
function outputChatBoxR(message, toElement, forceLog)
	if toElement == false then
        ...
        exports['cr_core']:sendMessageToAdmin(toElement, syntax .. message, 10)
...
function runString(commandstring, outputTo, source)
	...
	local sourceName
	if source then
		sourceName = exports['cr_admin']:getAdminName(source)
	else
		sourceName = "Console"
	end
	...
	get = getElementData
    me = source
	id = function(a) return exports['cr_core']:findPlayer(localPlayer, tostring(a)) end
...
addCommandHandler("crun", 
    function(player, command, ...)
        ...
        if player then
            ...
            outputDebugString("CRUN -> "..exports['cr_admin']:getAdminName(player).." lefuttata: "..tostring(commandstring), 0, 87, 255, 255)
