...
function Clear()
    phoneMessagesSearchCache = nil
    settingsSearchCache = nil
    ringTonesSearchCache = nil
    phoneContactsSearchCache = nil
    callHistorySearchCache = nil
    textbars = {}
        ...
        if s == "down" then
            ...
            for k,v in pairs(textbars) do
                ...
                if exports["cr_core"]:isInSlot(x,y,w,h, specBox[k]) and not adIconHover then
                    ...
                    if now == "messages.search" then
                        phoneMessagesSearchCache = nil
                    elseif now == "settings.search" then 
                        settingsSearchCache = nil
                    elseif now == "ringtoneSelector.search" then
                        ringTonesSearchCache = nil
                    elseif now == "contacts.search" then 
                        phoneContactsSearchCache = nil
                    elseif now == "callHistory.search" then
                        callHistorySearchCache = nil
                    end
...
function onGuiChange()
        ...
        if textbars[now][2][2]:gsub(" ", "") ~= "" then 
            ...
            for i = 1, #availableSettings do 
                ...
                if v then 
                    ...
                    if string.lower(tostring(visibleName)):find(text) then 
                        table.insert(settingsSearchCache, v)
                    end
                end
            end
        else
    ...
    elseif now == "ringtoneSelector.search" then 
        if textbars[now][2][2]:gsub(" ", "") ~= "" then 
            ringTonesSearchCache = {}

            local text = string.lower(tostring(textbars[now][2][2]))

            for i = 1, #availableRingtones do 
                local v = availableRingtones[i]

                if v then 
                    local name = v.name

                    if string.lower(tostring(name)):find(text) then 
                        table.insert(ringTonesSearchCache, v)
                    end
                end
            end
        else
    ...
    elseif now == "callHistory.search" then
        if textbars[now][2][2]:gsub(" ", "") ~= "" then
            callHistorySearchCache = {}
            
            local text = string.lower(textbars[now][2][2])

            for i = 1, #phoneData.callHistory do 
                local v = phoneData.callHistory[i]
                ...
                if v then 
                    ...
                    if string.lower(tostring(text2)):find(text) or string.lower(text3):find(text) then
                        table.insert(callHistorySearchCache, v)
                    end
                end
            end
        else
