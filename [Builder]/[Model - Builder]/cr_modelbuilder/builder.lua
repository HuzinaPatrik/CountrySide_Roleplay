...
function encryptModels(resource, model, paths, glassLoad, lodDistance)
    ...
    table.insert(cache, {model, outputFileName, paths.type, glassLoad, lodDistance, #randomHeader, #file})
    
    return true;
end

addCommandHandler("encryptModels", function(localPlayer)
    ...
    local resource = getResourceFromName("cr_modelpanel");
    resource:stop()    
    ...
    encryptTimer = setTimer(
        function()
            ...
            if child then 
                ...
                local lodDistance = child:getAttribute("lodDistance")
                if lodDistance and tonumber(lodDistance) then
                ...
                if modelid then
                    ...
                    --outputChatBox("asd")
                    if encryptModels(resource, modelid, paths, glassLoad, lodDistance) then
