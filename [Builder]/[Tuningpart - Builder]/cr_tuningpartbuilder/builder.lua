...
function encryptModels(resource, model, paths, glassLoad, virtualSkin)
	...
	local buildPath = ":cr_tuningparts/";
...
addCommandHandler("encryptTuningparts", function(localPlayer)
    ...
    local resource = getResourceFromName("cr_tuningpartpanel");
    resource:stop()    
    ...
    encryptTimer = setTimer(
        function()
            ...
            if child then 
                ...
                local resource = getResourceFromName("cr_tuningparts");
                if resource then
