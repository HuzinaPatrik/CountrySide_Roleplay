...
function setupViewer(position)
	if not cameraInteraction then 
		cameraInteraction = true 
		Viewer.cam.matrix.start = {
			position = position+Vector3(0, 4, 1),
			lookAt = position,
		}
...
function cameraMoveStart(b, state, x, y)
	if(not isCameraAdjusting and cameraInteraction) then
		if b == "left" and state == "down" then
			--if(isCursorHover(0, cDetails.render.hUp, s.x, s.y-cDetails.render.hDown-75)) then
			if not hover and not isCursorInPanel then 
