...
function addEventHandler(eventName, attachedTo, handlerFunction, getPropagated, priority, customName)
    ...
    if eventName == "onClientRender" and customName then
        renderTimers[customName] = setTimer(handlerFunction, 15, 0)
        return
    end
...
function removeEventHandler(eventName, attachedTo, functionVar)
    ...
    if eventName == "onClientRender" then
        for k,v in pairs()
        --local timer = setTimer(handlerFunction, 15, 0)
        --table.insert(renderTimers, 1, timer)
    end
